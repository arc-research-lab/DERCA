...
void address_A_ddr(axis_stream_32& addrA_out,const int TX,const int TY,const int TZ,
    ...
    int start = resume?start_rd-2:start_rd;
...
void compute(axis_stream_512& dataA_out, axis_stream_512& dataB_out, axis_stream_256& dataC_in,
    ...
    /*only clean the output buffer at the preemption*/
    const int Total_rd=TX*TY*TZ;
    if(preemption || start_rd==0){
        for(int x = 0; x < X*Z; x++){
            for(int j=0;j<PACKET_NUM;j++){
                for (int i = 0; i < OUT_SIZE; i++){
                    #pragma HLS PIPELINE II = 1
    ...
    if(resume){//need to re-load and re-comp to resume the pipeline
        start_rd_with_resume -= 2;
    }
    for (int rd=start_rd_with_resume; rd<end_rd;rd++){
        ...
        bool enable_comp_resume = (rd>=start_rd-1);
...
void dma0(ap_uint<AXI_WIDTH_512>* ina, ap_uint<AXI_WIDTH_512>* inb, ap_uint<AXI_WIDTH_256>* out0, 
            const int TX, const int TY, const int TZ,
            int start_rd, int end_rd, bool preemption, bool resume,
            axis_stream& txA_0,  axis_stream& txA_1,  axis_stream& txA_2,  axis_stream& txA_3,
            axis_stream& txA_4,  axis_stream& txA_5,  axis_stream& txA_6,  axis_stream& txA_7,
...
void dma1(ap_uint<AXI_WIDTH_512>* ina, ap_uint<AXI_WIDTH_512>* inb, ap_uint<AXI_WIDTH_256>* out0, 
        const int TX, const int TY, const int TZ,
        int start_rd, int end_rd, bool preemption, bool resume,
        axis_stream& txA_0,  axis_stream& txA_1,  axis_stream& txA_2,  axis_stream& txA_3,
        axis_stream& txA_4,  axis_stream& txA_5,  axis_stream& txA_6,  axis_stream& txA_7,
        ...
        axis_stream& rxC_16, axis_stream& rxC_17, axis_stream& rxC_18, axis_stream& rxC_19,
        axis_stream& rxC_20, axis_stream& rxC_21, axis_stream& rxC_22, axis_stream& rxC_23,
        int iter, hls::stream<int>& record_s){
for(int i=0; i<iter; i++){
    record_s.write(0);
    dma0(ina, inb, out0, TX, TY, TZ,
        start_rd,end_rd, preemption,resume,
        txA_0,  txA_1,  txA_2,  txA_3,
        txA_4,  txA_5,  txA_6,  txA_7,
        txA_8,  txA_9,  txA_10, txA_11,
...
void record(long* record_array, hls::stream<int>& record_s, int iter){
    int record_idx=0;
    record_loop:
    for(long i=0;;i++){
        #pragma HLS pipeline II=1
        if(!record_s.empty()){
            record_s.read();
            record_array[record_idx]=i;
            record_idx++;
        }
        else if(record_idx>=2*iter-1){
            break;
        }
    }
}
...
void dma(ap_uint<AXI_WIDTH_512>* ina, ap_uint<AXI_WIDTH_512>* inb, ap_uint<AXI_WIDTH_256>* out0, 
    ...
    hls::stream<int> record_s("stream for recording trace");
    #pragma HLS dataflow

    dma1(ina, inb, out0, TX, TY, TZ,
        start_rd,end_rd,preemption,resume,
        txA_0,  txA_1,  txA_2,  txA_3,
        txA_4,  txA_5,  txA_6,  txA_7,
        txA_8,  txA_9,  txA_10, txA_11,
    ...
    record(record_array,record_s,iter);
}
