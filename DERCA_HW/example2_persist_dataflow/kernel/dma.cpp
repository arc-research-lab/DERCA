...
void address_A_ddr(axis_stream_32& addrA_out,const int TX,const int TY,const int TZ,
                    int start_rd, int end_rd, bool preemption, bool resume) {
    /*instead of load all of a MM, we load a small piece according to the index*/
    #pragma HLS inline off
    int start = resume?start_rd-1:start_rd;
    int end = end_rd<TX*TY*TZ?end_rd:TX*TY*TZ;
    for(ap_uint<32> rd=start; rd<end; rd++){
        ap_uint<32> pos;
        int ty = rd%TY;
        int tz = rd/TY%TZ;
        int tx = rd/TY/TZ;
...
void loadA_ddr(ap_uint<AXI_WIDTH_512>* ina, axis_stream_32& addrA_in,axis_stream_512& dataA_out,
    const int TX,const int TY,const int TZ,
    int start_rd, int end_rd, bool preemption, bool resume) {
    #pragma HLS inline off
    int start = resume?start_rd-1:start_rd;
    int end = end_rd<TX*TY*TZ?end_rd:TX*TY*TZ;
    int Num_rd = end-start;
    ap_uint<512> temp_data;
    ap_uint<32> bound=boundA*Num_rd;
...
void loadB_ddr(ap_uint<AXI_WIDTH_512>* inb, axis_stream_32& addrB_in,axis_stream_512& dataB_out,
    const int TX,const int TY,const int TZ,
    int start_rd, int end_rd, bool preemption, bool resume) {
    #pragma HLS inline off
    int start = resume?start_rd-1:start_rd;
    ...
    int Num_rd = end-start;
    ap_uint<512> temp_data;
    ap_uint<32> bound=boundB*Num_rd;
    for(ap_uint<32> i=0;i<bound;i++){
    #pragma HLS PIPELINE II = 1
...
void address_C_ddr(axis_stream_32& addrC_out,const int TX,const int TY, const int TZ,
    int start_rd, int end_rd, bool preemption, bool resume) {
    #pragma HLS inline off
    //Need to find out how many outputs are there when given a start and end rd
    //output rd>TY&&(rd-2)%TY==(TY-1)
    ...
    //find the first rd that is (1) generating output and (2) larger equal than start
    //then add the rd by TY
    int k ;
    if(start_rd>=2){
        k = (start_rd - 2) / TY;}
    else{
        k = 0;}
    int first_output = (TY + 1) + k * TY;

    for(int rd=first_output;rd<end_rd;rd+=TY){
        int tz = (rd-2)/TY%TZ;
        int tx = (rd-2)/TY/TZ;
        ap_uint<32> pos;
        for(ap_uint<32> j=0;j<N;j++){
...
void storeC_ddr(ap_uint<AXI_WIDTH_256>* outc, axis_stream_32& addrC_in,axis_stream_256& dataC_in,
                const int TX,const int TY,const int TZ,
                int start_rd, int end_rd, bool preemption, bool resume) { 
#pragma HLS inline off
...
//for persisting intermediate data
void address_C_persist(axis_stream_32& addrC_out,const int TX,const int TY, const int TZ,
    int start_rd, int end_rd, bool preemption, bool resume) {
    #pragma HLS inline off
    //during persist, we only need to store one tile to a fixed pos
    //equal to tx=0, ty=0, TX=1
    ap_uint<32> pos;
    //only persist one tile when preemption happens
    if(preemption){
        for(ap_uint<32> j=0;j<N;j++){
            for(ap_uint<32> i=0;i<M/C_PER_TRA;i++){
            #pragma HLS PIPELINE II = 1
                pos=i+j*(M/C_PER_TRA)*1;
                addrC_out.write(pos);
            }
        }
    }
...
void persistC_ddr(ap_uint<AXI_WIDTH_256>* outc, axis_stream_32& addrC_in,axis_stream_256& dataC_in,
                const int TX,const int TY,const int TZ,
                int start_rd, int end_rd, bool preemption, bool resume) { 
#pragma HLS inline off
    if(preemption){
...
void persistC(axis_stream_256& dataC_out, ap_uint<PLIO_WIDTH> c_buf[A*C/PACKET_NUM][X*Z][PACKET_NUM][OUT_SIZE], bool enable){
#pragma HLS inline off
    if(enable){
        for(int z=0;z<Z;z++){
            for(int c=0;c<C;c++){
...
void address_C_resume(axis_stream_32& addrC_out,const int TX,const int TY, const int TZ,
    int start_rd, int end_rd, bool preemption, bool resume) {
    #pragma HLS inline off
    //during persist, we only need to store one tile to a fixed pos
    //equal to tx=0, ty=0, TX=1
...
void resumeC_ddr(ap_uint<AXI_WIDTH_256>* outc, axis_stream_32& addrC_in,axis_stream_256& dataC_out,
                const int TX,const int TY,const int TZ,
                int start_rd, int end_rd, bool preemption, bool resume) { 
#pragma HLS inline off
    if(resume){
        for(ap_uint<32> i=0;i<boundC;i++){
            #pragma HLS PIPELINE II = 1
                ap_uint<32> addr = addrC_in.read();
                ap_uint<256> temp_data = outc[addr];
                dataC_out.write(temp_data);
        }
    }
}

void resumeC(axis_stream_256& dataC_in, ap_uint<PLIO_WIDTH> c_buf[A*C/PACKET_NUM][X*Z][PACKET_NUM][OUT_SIZE], bool enable){
#pragma HLS inline off
    if(enable){
        for(int z=0;z<Z;z++){
            for(int c=0;c<C;c++){
                    ...
                    for(int x=0;x<X;x++){
                        for (int a=0;a<A;a++){
                            for (int n=0; n<H1/C_PER_TRA;n++){
                                ...
                                temp_data = dataC_in.read();
                                // temp_data(127,0)=c_buf[pos3][pos2][pos1][pos0];
                                // temp_data(255,128)=c_buf[pos3][pos2][pos1][pos0+1];
                                c_buf[pos3][pos2][pos1][pos0] = temp_data(127,0);
                                c_buf[pos3][pos2][pos1][pos0+1] = temp_data(255,128);
                            }
                        }
                    }
                }
...
void compute(axis_stream_512& dataA_out, axis_stream_512& dataB_out, axis_stream_256& dataC_in,
            const int TX, const int TY, const int TZ,
            int start_rd, int end_rd, bool preemption, bool resume, 
            int last_rd, ap_uint<AXI_WIDTH_256>* persist_buf,
            axis_stream_256& dataC_persist, axis_stream_256& dataC_resume,
    ...
    if(preemption){
        int c_flg=((last_rd-1)/TY)%2;//0:partial result to buf0
        //assume in the last iteration, write to bufC_0, then persist bufC_0
        //always persist data 
        int ty = last_rd%TY;
        int tz = last_rd/TY%TZ;
        int tx = last_rd/TY/TZ;
        if(c_flg==0){
            persistC(dataC_persist, buff0_C, 1);
        }
        else{//c_flg==1
            persistC(dataC_persist, buff1_C, 1);
        }
    }

    int start_rd_with_resume = start_rd;
    /*upon resuming, need to loadback the C buf, and resume the pipeline by reloading*/
    if(resume){
        //use the start_rd - 1 as the idx of the last rd in the last region
        start_rd_with_resume -= 1;
        int c_flg=((start_rd_with_resume-1)/TY)%2;
        int ty = start_rd_with_resume%TY;
        int tz = start_rd_with_resume/TY%TZ;
        int tx = start_rd_with_resume/TY/TZ;
        if(c_flg==0){
            resumeC(dataC_resume, buff0_C, 1);
        }
        else{//c_flg==1
            resumeC(dataC_resume, buff1_C, 1);
        }
        //leverage the main loop for load back input
    }
    for (int rd=start_rd_with_resume; rd<end_rd;rd++){
        int c_flg=0,s_flg=0;
        int tx_A=0,ty_A=0;
        int ty_B=0,tz_B=0;
        int tx_C=0,tz_C=0;
        ...
        bool enable_comp_resume = (rd>=start_rd);
        bool enable_store_resume = (rd>=start_rd);
        if(rd>0){
            c_flg=((rd-1)/TY)%2;
        }
        if(rd>1){
        ...
        if(rd%2==0&&c_flg==0){
            ...
            sendA<0>(buff1_A[0],txA_0,txA_1,txA_2,txA_3,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<1>(buff1_A[1],txA_4,txA_5,txA_6,txA_7,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<2>(buff1_A[2],txA_8,txA_9,txA_10,txA_11,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<3>(buff1_A[3],txA_12,txA_13,txA_14,txA_15,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<4>(buff1_A[4],txA_16,txA_17,txA_18,txA_19,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<5>(buff1_A[5],txA_20,txA_21,txA_22,txA_23,rd>0&&rd<Total_rd+1&&enable_comp_resume);

            sendB<0>(buff1_B[0],txB_0,txB_1,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<1>(buff1_B[1],txB_2,txB_3,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<2>(buff1_B[2],txB_4,txB_5,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<3>(buff1_B[3],txB_6,txB_7,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<4>(buff1_B[4],txB_8,txB_9,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<5>(buff1_B[5],txB_10,txB_11,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<6>(buff1_B[6],txB_12,txB_13,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<7>(buff1_B[7],txB_14,txB_15,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<8>(buff1_B[8],txB_16,txB_17,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<9>(buff1_B[9],txB_18,txB_19,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<10>(buff1_B[10],txB_20,txB_21,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<11>(buff1_B[11],txB_22,txB_23,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<12>(buff1_B[12],txB_24,txB_25,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<13>(buff1_B[13],txB_26,txB_27,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<14>(buff1_B[14],txB_28,txB_29,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<15>(buff1_B[15],txB_30,txB_31,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            
            reshapeC<0>(buff0_C[0],rxC_0,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<1>(buff0_C[1],rxC_1,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<2>(buff0_C[2],rxC_2,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<3>(buff0_C[3],rxC_3,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<4>(buff0_C[4],rxC_4,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<5>(buff0_C[5],rxC_5,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<6>(buff0_C[6],rxC_6,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<7>(buff0_C[7],rxC_7,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<8>(buff0_C[8],rxC_8,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<9>(buff0_C[9],rxC_9,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<10>(buff0_C[10],rxC_10,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<11>(buff0_C[11],rxC_11,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<12>(buff0_C[12],rxC_12,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<13>(buff0_C[13],rxC_13,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<14>(buff0_C[14],rxC_14,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<15>(buff0_C[15],rxC_15,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<16>(buff0_C[16],rxC_16,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<17>(buff0_C[17],rxC_17,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<18>(buff0_C[18],rxC_18,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<19>(buff0_C[19],rxC_19,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<20>(buff0_C[20],rxC_20,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<21>(buff0_C[21],rxC_21,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<22>(buff0_C[22],rxC_22,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<23>(buff0_C[23],rxC_23,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            storeC(dataC_in, buff1_C, rd>TY&&s_flg==(TY-1)&&enable_store_resume);
        }
        else if(rd%2==1&&c_flg==0){
            loadA(dataA_out,buff1_A,rd<Total_rd);
            loadB(dataB_out,buff1_B,rd<Total_rd);
            sendA<0>(buff0_A[0],txA_0,txA_1,txA_2,txA_3,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<1>(buff0_A[1],txA_4,txA_5,txA_6,txA_7,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<2>(buff0_A[2],txA_8,txA_9,txA_10,txA_11,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<3>(buff0_A[3],txA_12,txA_13,txA_14,txA_15,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<4>(buff0_A[4],txA_16,txA_17,txA_18,txA_19,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendA<5>(buff0_A[5],txA_20,txA_21,txA_22,txA_23,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<0>(buff0_B[0],txB_0,txB_1,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<1>(buff0_B[1],txB_2,txB_3,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<2>(buff0_B[2],txB_4,txB_5,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<3>(buff0_B[3],txB_6,txB_7,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<4>(buff0_B[4],txB_8,txB_9,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<5>(buff0_B[5],txB_10,txB_11,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<6>(buff0_B[6],txB_12,txB_13,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<7>(buff0_B[7],txB_14,txB_15,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<8>(buff0_B[8],txB_16,txB_17,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<9>(buff0_B[9],txB_18,txB_19,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<10>(buff0_B[10],txB_20,txB_21,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<11>(buff0_B[11],txB_22,txB_23,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<12>(buff0_B[12],txB_24,txB_25,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<13>(buff0_B[13],txB_26,txB_27,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<14>(buff0_B[14],txB_28,txB_29,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<15>(buff0_B[15],txB_30,txB_31,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<0>(buff0_C[0],rxC_0,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<1>(buff0_C[1],rxC_1,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<2>(buff0_C[2],rxC_2,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<3>(buff0_C[3],rxC_3,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            ...
            sendB<14>(buff1_B[14],txB_28,txB_29,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            sendB<15>(buff1_B[15],txB_30,txB_31,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<0>(buff1_C[0],rxC_0,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<1>(buff1_C[1],rxC_1,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<2>(buff1_C[2],rxC_2,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<3>(buff1_C[3],rxC_3,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<4>(buff1_C[4],rxC_4,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<5>(buff1_C[5],rxC_5,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<6>(buff1_C[6],rxC_6,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<7>(buff1_C[7],rxC_7,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<8>(buff1_C[8],rxC_8,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<9>(buff1_C[9],rxC_9,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<10>(buff1_C[10],rxC_10,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<11>(buff1_C[11],rxC_11,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<12>(buff1_C[12],rxC_12,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<13>(buff1_C[13],rxC_13,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<14>(buff1_C[14],rxC_14,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<15>(buff1_C[15],rxC_15,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<16>(buff1_C[16],rxC_16,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<17>(buff1_C[17],rxC_17,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<18>(buff1_C[18],rxC_18,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<19>(buff1_C[19],rxC_19,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<20>(buff1_C[20],rxC_20,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<21>(buff1_C[21],rxC_21,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<22>(buff1_C[22],rxC_22,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            reshapeC<23>(buff1_C[23],rxC_23,rd>0&&rd<Total_rd+1&&enable_comp_resume);
            storeC(dataC_in, buff0_C, rd>TY&&s_flg==(TY-1)&&enable_store_resume);
        }
...
extern "C"{
void dma(ap_uint<AXI_WIDTH_512>* ina, ap_uint<AXI_WIDTH_512>* inb, ap_uint<AXI_WIDTH_256>* out0, 
            const int TX, const int TY, const int TZ,
            int start_rd, int end_rd, bool preemption, bool resume,
            int last_rd, ap_uint<AXI_WIDTH_256>* persist_buf,
            axis_stream& txA_0,  axis_stream& txA_1,  axis_stream& txA_2,  axis_stream& txA_3,
...
{    
    ...
    #pragma HLS interface s_axilite bundle=control port=start_rd
    #pragma HLS interface s_axilite bundle=control port=end_rd
    #pragma HLS interface s_axilite bundle=control port=preemption
    #pragma HLS interface s_axilite bundle=control port=resume
    #pragma HLS interface s_axilite bundle=control port=last_rd
    #pragma HLS interface m_axi offset=slave bundle=gmem4 port=persist_buf max_write_burst_length=64 num_write_outstanding=64
    #pragma HLS interface s_axilite bundle=control port=persist_buf
    ...
    // #pragma HLS INTERFACE m_axi port = record_array offset = slave bundle = gmem3
    // #pragma HLS interface s_axilite bundle=control port=record_array
    #pragma HLS dataflow
    ...
    axis_stream_32 addrC_persist;
    axis_stream_32 addrC_resume;
    axis_stream_256 dataC_persist;
    axis_stream_256 dataC_resume;
     
    address_A_ddr(addrA_out,TX,TY,TZ,start_rd,end_rd, preemption,resume);
    loadA_ddr(ina, addrA_out,dataA_out,TX,TY,TZ,start_rd,end_rd, preemption,resume);

    address_B_ddr(addrB_out,TX,TY,TZ,start_rd,end_rd, preemption,resume);
    loadB_ddr(inb,addrB_out,dataB_out,TX,TY,TZ,start_rd,end_rd, preemption,resume);

    address_C_ddr(addrC_out,TX,TY,TZ,start_rd,end_rd, preemption,resume);
    storeC_ddr(out0,addrC_out,dataC_in,TX,TY,TZ,start_rd,end_rd, preemption,resume);

    address_C_persist(addrC_persist,TX,TY,TZ,start_rd,end_rd, preemption,resume);
    persistC_ddr(persist_buf,addrC_persist,dataC_persist,TX,TY,TZ,start_rd,end_rd, preemption,resume);

    address_C_resume(addrC_resume,TX,TY,TZ,start_rd,end_rd, preemption,resume);
    resumeC_ddr(persist_buf,addrC_resume,dataC_resume,TX,TY,TZ,start_rd,end_rd, preemption,resume);
    
    compute(dataA_out, dataB_out, dataC_in, TX, TY, TZ,
            start_rd,end_rd, preemption,resume,
            last_rd, persist_buf,
            dataC_persist,dataC_resume,
